...
<div class="medium-12 columns">
    ...
    <form method="post" enctype="multipart/form-data">
        ...
        <select name="category">
            ...
            @if(!Input::old('category')) selected @endif>Categorie</option>
            <option value="fotografie"
            @if(Input::old('category')=='fotografie') selected @endif>Fotografie</option>
            <option value="logo"
            @if(Input::old('category')=='logo') selected @endif>Logo design</option>
            <option value="illustratie"
            @if(Input::old('category')=='illustratie') selected @endif>Illustratie</option>
            <option value="tekenen"
            @if(Input::old('category')=='tekenen') selected @endif>Tekenen</option>
            <option value="webdesign"
            @if(Input::old('category')=='webdesign') selected @endif>Webdesign</option>
            <option value="webdevelopment"
            @if(Input::old('category')=='webdevelopment') selected @endif>Webdevelopment</option>
        </select>
